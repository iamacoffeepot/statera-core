...
import {LendingTermsPacked, UQ4x4} from "../src/types/Types.sol";

contract LendingTermsLibraryTest is Test {
    function test_is_valid_borrow_factor() external {
        for (
            UQ4x4 borrowFactor = LendingTermsLibrary.BORROW_FACTOR_MINIMUM;
            borrowFactor <= LendingTermsLibrary.BORROW_FACTOR_MAXIMUM;
        ) {
            assertTrue(LendingTermsLibrary.isValidBorrowFactor(borrowFactor));
            unchecked { borrowFactor = UQ4x4.wrap(UQ4x4.unwrap(borrowFactor) + 1); }
        }
    }

    function test_is_valid_profit_factor() external {
        for (
            UQ4x4 profitFactor = LendingTermsLibrary.PROFIT_FACTOR_MINIMUM;
            profitFactor <= LendingTermsLibrary.PROFIT_FACTOR_MAXIMUM;
        ) {
            assertTrue(LendingTermsLibrary.isValidProfitFactor(profitFactor));
            unchecked { profitFactor = UQ4x4.wrap(UQ4x4.unwrap(profitFactor) + 1); }
        }
    }

    function test_fuzz_try_pack_unpack(UQ4x4 borrowFactor, UQ4x4 profitFactor) external {
        ...
        (UQ4x4 unpackedBorrowFactor, UQ4x4 unpackedProfitFactor) = LendingTermsLibrary.unpack(terms);
